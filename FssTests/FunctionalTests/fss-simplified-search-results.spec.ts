...
import { ClickWaitRetry, SearchAttribute} from '../Assets/Helper/SearchPageHelper';
import { stringOperatorList, symbolOperatorListForFileSize, symbolOperatorListForDate } from '../Assets/Helper/ConstantHelper'
...
test.describe('Test Search Result Scenario On Simplified Search Page', () => {
  ...
  test('Verify batch attributes with multiple values are displayed on filter panel', async ({ page }) => {
    ...
    //const configuredBatchAttibutes = await page.$$eval(fssSearchPageObjectsConfig.filterBatchAttributes, elements => { return elements.map(element => element.textContent) });  //rhz
